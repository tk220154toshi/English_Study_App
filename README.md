...
> 📱💻 **3段階レスポンシブ（1コードで全端末）**
> - **スマホ**：1画面完結のアプリUI。**☰ 左ドロワー**でモード切替、**📋 問題**と
>   **🖥 出力**はボトムシート、**📊** で XP/実績シート（コマンドモードでは問題がロボットに
>   重なって表示＝タップで切替）。縦スクロール不要。
> - **iPad（タブレット）**：タッチ操作はそのままに大画面を活用。**左モードレール＋問題パネル
>   ＋エディタ＋下部の出力ドック**を同時表示、統計は 📊 の右スライドシート。
> - **PC**：VSCode 風の4カラム（サイドバー＋エディタ＋統計）。
> どれも同じ `public/` が画面サイズで自動切替（回転OK・iOSセーフエリア対応）。
...
## 🧭 プラットフォーム構成（iOS / iPadOS / macOS の分け方）

**結論：リポジトリやブランチを OS ごとに分ける必要はありません。** むしろ分けると
共有コードが二重管理になり不利です。この構成では「**共有コードは git／プラットフォーム
固有はビルド層**」で分離しています（Capacitor 標準の推奨形）。

```
リポジトリ（git管理・全OS共通）
├─ public/            画面・ロジック（レスポンシブで phone/tablet/desktop 自動切替）
├─ server/            ローカル開発用サーバー（本番のダイレクトモードでは不要）
├─ capacitor.config.json
└─ ...
ネイティブプロジェクト（各マシンで生成・.gitignore 済み＝コミットしない）
├─ ios/          → iPhone と iPad を1つの「ユニバーサル」ターゲットでカバー
│                  （macOS は同じ ios ターゲットに Mac Catalyst を足すだけ）
└─ electron/     → Mac/Win/Linux デスクトップ（任意）
```

- **iOS と iPadOS は分けません**：Capacitor の `ios` ターゲットはユニバーサル
  （iPhone＋iPad 両対応）。UI は 3 段階レスポンシブが端末サイズで自動切替。
- **macOS も基本は分けません**：同じ `ios` プロジェクトに Mac Catalyst の実行先を
  追加するだけ。独立配布したい場合のみ Electron を別途生成（`electron/` は gitignore）。
- **OS 固有の分岐が必要になったら**：ファイルを分けず、実行時に
  `Capacitor.getPlatform()`（`'ios'` / `'electron'` / `'web'`）で切り替えます。
- ネイティブ側の設定（署名・アイコン・Info.plist 等）は各 `ios/`・`electron/` 内に
  生成され、gitignore 済み。もし署名設定などを git で保全したい場合だけ、対象を
  `.gitignore` から外して個別にコミットしてください（OSごとの分割ではなく、
  同一リポジトリ内に共存）。

> まとめ：**1リポジトリ＝1コードベースのまま、iPhone / iPad / Mac を出せます。**
> 分割は不要（推奨しません）。
